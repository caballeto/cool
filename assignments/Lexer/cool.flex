...
%x NESTED_COMMENT LINE_COMMENT STRING STRING_ERROR
...
<STRING>\"		{ cool_yylval.symbol = new StringEntry(strdup(string_buf), str_len, str_count++); BEGIN(INITIAL); return STR_CONST; }
<STRING>\\\n		{ if (!is_valid(str_len)) {
			     BEGIN(STRING_ERROR); 
			     return ERROR;
			   }
...
<STRING>\\.		{ if (!is_valid(str_len)) {
			  ...
			  switch(yytext[1]) {
			    ...
			    default: string_buf[str_len++] = yytext[1]; break;
			  }
			 } 

\"			{ str_len = 0; BEGIN(STRING); }

<STRING>\n		{ cool_yylval.error_msg = "Unterminated string constant"; curr_lineno++; BEGIN(INITIAL); return ERROR; }

<STRING><<EOF>>		{ cool_yylval.error_msg = "EOF in string constant"; BEGIN(STRING_ERROR); return ERROR; }

<STRING>\0		{ cool_yylval.error_msg = "String contains null character"; BEGIN(STRING_ERROR); return ERROR; }
